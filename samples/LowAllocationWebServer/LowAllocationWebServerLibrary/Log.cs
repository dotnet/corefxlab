    ...
    public static class HttpLogExtensions
    {
        public static void LogRequest(this Log log, HttpRequest request)
        {
            if (log.IsVerbose)
            {
                ...
                log.LogMessage(Log.Level.Verbose, "\tMethod:       {0}", request.Verb.ToUtf8String(TextEncoder.InvariantUtf8));
                log.LogMessage(Log.Level.Verbose, "\tRequest-URI:  {0}", request.Path.ToUtf8String(TextEncoder.InvariantUtf8));
                log.LogMessage(Log.Level.Verbose, "\tHTTP-Version: {0}", request.Version.ToUtf8String(TextEncoder.InvariantUtf8));

                log.LogMessage(Log.Level.Verbose, "\tHttp Headers:");
                var position = Position.First;
                while(request.Headers.TryGet(ref position, out var header, true))
                {
                    log.LogMessage(Log.Level.Verbose, "\t\t{0}: {1}", header.Name.ToUtf8String(TextEncoder.InvariantUtf8), header.Value.ToUtf8String(TextEncoder.InvariantUtf8));
                }

                var body = request.Body.ToString(TextEncoder.InvariantUtf8);               
