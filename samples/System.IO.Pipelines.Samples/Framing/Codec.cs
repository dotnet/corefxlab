...
namespace System.IO.Pipelines.Samples.Framing
{
    ...
    {
        public bool TryDecode(ref ReadOnlySequence<byte> input, out Line frame)
        {
            if (input.TrySliceTo((byte)'\r', (byte)'\n', out ReadOnlySequence<byte> slice, out SequencePosition cursor))
            {
    ...
    {
        bool TryDecode(ref ReadOnlySequence<byte> input, out TInput frame);
    }
