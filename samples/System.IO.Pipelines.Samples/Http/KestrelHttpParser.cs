    ...
    public class KestrelHttpParser : IHttpParser
    {
        ...
        private static bool TryGetNewLineSpan(ref ReadableBuffer buffer, ref Span<byte> span, out ReadCursor end)
        {
            ...
            if (ReadCursorOperations.Seek(start, buffer.End, out end, ByteLF) != -1)
            {
                ...
                span = buffer.Slice(start, end).ToArray();
                return true;
            }
            return false;
        }
        ...
        private InvalidDataException GetRejectRequestLineException(Span<byte> span)
        {
            return new InvalidDataException();
        }

        private unsafe InvalidDataException GetRejectUnknownVersion(byte* data, int length, int versionStart)
        {
        ...
        private InvalidDataException GetRejectRequestHeaderException(Span<byte> span)
        {
            return new InvalidDataException();
        }
