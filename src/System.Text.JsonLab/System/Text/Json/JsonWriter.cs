...
namespace System.Text.JsonLab
{
    public ref struct Utf8JsonWriter<TBufferWriter> where TBufferWriter : IBufferWriter<byte>
    {
        ...
        private void WriteStartUtf16Pretty(ReadOnlySpan<byte> nameSpanByte, byte token)
        {
            ...
            if (Buffers.Text.TextEncodings.Utf16.ToUtf8Length(nameSpanByte, out int bytesNeededValue) != OperationStatus.Done)
            {
            ...
            OperationStatus status = Buffers.Text.TextEncodings.Utf16.ToUtf8(nameSpanByte, byteBuffer.Slice(idx), out int consumed, out int written);
        ...
        private void WriteAttributeUtf8Pretty(ReadOnlySpan<byte> nameSpan, ReadOnlySpan<byte> valueSpan)
        {
            ...
            if (Buffers.Text.TextEncodings.Utf16.ToUtf8Length(nameSpan, out int bytesNeededName) != OperationStatus.Done)
            {
                JsonThrowHelper.ThrowArgumentExceptionInvalidUtf8String();
            }
            if (Buffers.Text.TextEncodings.Utf16.ToUtf8Length(valueSpan, out int bytesNeededValue) != OperationStatus.Done)
            {
                JsonThrowHelper.ThrowArgumentExceptionInvalidUtf8String();
            }
        ...
        private void WriteAttributeUtf8Pretty(ReadOnlySpan<byte> nameSpanByte, ReadOnlySpan<byte> valueSpanByte, int bytesNeeded)
        {
            ...
            status = Buffers.Text.TextEncodings.Utf16.ToUtf8(valueSpanByte, byteBuffer.Slice(idx), out consumed, out written);
        ...
        private void WriteAttributeUtf8Pretty(ReadOnlySpan<byte> nameSpan, long value)
        {
            ...
            OperationStatus status = Buffers.Text.TextEncodings.Utf16.ToUtf8(nameSpan, byteBuffer.Slice(idx), out int consumed, out int written);
            if (status != OperationStatus.Done)
            {
                JsonThrowHelper.ThrowFormatException();
            }
        ...
        private void WriteValueUtf8Pretty(ReadOnlySpan<byte> valueSpanByte)
        {
            ...
            OperationStatus status = Buffers.Text.TextEncodings.Utf16.ToUtf8(valueSpanByte, byteBuffer.Slice(idx), out int consumed, out int written);
            if (status != OperationStatus.Done)
            {
                JsonThrowHelper.ThrowFormatException();
            }
        ...
        private int CalculateValueBytesNeededPretty(ReadOnlySpan<byte> span)
        {
            ...
            if (Buffers.Text.TextEncodings.Utf16.ToUtf8Length(span, out int bytesNeededValue) != OperationStatus.Done)
            {
                JsonThrowHelper.ThrowArgumentExceptionInvalidUtf8String();
            }
            bytesNeeded += bytesNeededValue;
        ...
        private int CalculateStartAttributeBytesNeededPretty(ReadOnlySpan<byte> nameSpan)
        {
            ...
            if (Buffers.Text.TextEncodings.Utf16.ToUtf8Length(nameSpan, out int bytesNeededValue) != OperationStatus.Done)
            {
                JsonThrowHelper.ThrowArgumentExceptionInvalidUtf8String();
            }
            bytesNeeded += bytesNeededValue;
