...
namespace System.Text.Parsing
{
    public static partial class InvariantParser
    {
		...
		public static bool TryParse(byte[] text, int startIndex, out long value, out int bytesConsumed)
        {
            ...
            for (int byteIndex = startIndex; byteIndex < text.Length; byteIndex++) // loop through the byte array
            {
                ...
                candidate += (text[byteIndex] - '0'); // parse the current digit to a long and add it to the temporary value
        ...
        unsafe public static bool TryParse(byte* text, int startIndex, out long value, out int bytesConsumed)
        {
            value = 0;
            bytesConsumed = 0;
            bool negative = false;
