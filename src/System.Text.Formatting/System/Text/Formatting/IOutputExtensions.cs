...
namespace System.Text.Formatting
{
    public static class IOutputExtensions
    {
        ...
        {
            if (!value.TryFormat(formatter.GetMemory().Span, out int bytesWritten, format, symbolTable))
            {
                return false;
            }
        ...
        {
            var result = symbolTable.TryEncode(value, formatter.GetMemory().Span, out int consumed, out int written);
            if (result)
        ...
        {
            if (!symbolTable.TryEncode(value, formatter.GetMemory().Span, out int consumed, out int bytesWritten))
            {
                return false;
            }
            formatter.Advance(bytesWritten);
        ...
        {
            if (!CustomFormatter.TryFormat(value, formatter.GetMemory().Span, out int bytesWritten, format, symbolTable))
            {
                return false;
            }
            formatter.Advance(bytesWritten);
