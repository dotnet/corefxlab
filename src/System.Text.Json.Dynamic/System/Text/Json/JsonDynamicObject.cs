...
namespace System.Text.Json
{
    public class JsonDynamicObject : DynamicObject, IBufferFormattable
    {
        ...
        public bool TryFormat(Span<byte> buffer, Format.Parsed format, EncodingData formattingData, out int written)
        {
            written = 0;
            int justWritten;
            if(!'{'.TryFormat(buffer, format, formattingData, out justWritten)){
                return false;
            }
            written += justWritten;
            ...
            foreach(var property in _properties)
            {
                ...
                {
                    if (!','.TryFormat(buffer.Slice(written), format, formattingData, out justWritten))
                    {
                        return false;
                    }
                    written += justWritten;
                ...
                if(!property.Key.TryFormat(buffer.Slice(written), format, formattingData, out justWritten))
                {
                ...
                written += justWritten;
                if (!property.Value.TryFormat(buffer.Slice(written), format, formattingData, out justWritten))
                {
                    written = 0; return false;
                }
                written += justWritten;
        ...
        struct JsonValue : IBufferFormattable
        {
            ...
            public bool TryFormat(Span<byte> buffer, Format.Parsed format, EncodingData formattingData, out int written)
            {
                switch (_type)
                {
                    ...
                    case JsonReader.JsonValueType.Null:
                        return "null".TryFormat(buffer, format, formattingData, out written);
                    case JsonReader.JsonValueType.True:
                        return "true".TryFormat(buffer, format, formattingData, out written);
                    case JsonReader.JsonValueType.False:
                        return "false".TryFormat(buffer, format, formattingData, out written);
                    default:
