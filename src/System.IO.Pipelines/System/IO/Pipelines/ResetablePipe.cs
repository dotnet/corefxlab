...
namespace System.IO.Pipelines
{
    ...
    /// </summary>
    public class ResetablePipe : Pipe, IAwaiter<ReadResult>, IAwaiter<FlushResult>
    {
        private const int SegmentPoolSize = 16;

        private static readonly Action<object> _signalReaderAwaitable = state => ((ResetablePipe)state).ReaderCancellationRequested();
        private static readonly Action<object> _signalWriterAwaitable = state => ((ResetablePipe)state).WriterCancellationRequested();
        ...
        {
            private readonly ResetablePipe _pipe;

            public ResetablePipeReader(ResetablePipe pipe)
            {
                _pipe = pipe;
            }
        ...
        private class ResetablePipeWriter : PipeWriter
        {
            ...
            public ResetablePipeWriter(ResetablePipe pipe)
            {
                _pipe = pipe;
            }
