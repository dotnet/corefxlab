...
namespace System.Text 
{
    public static partial class PrimitiveFormatter
    {
        ...
        public static bool TryFormatSlow(this DateTimeOffset value, Span<byte> buffer, out int bytesWritten, TextFormat format = default(TextFormat), TextEncoder encoder = null)
        {
            if (format.IsDefault)
            {
                format.Symbol = 'G';
            ...
            {
                case 'R':
                    return TryFormatDateTimeRfc1123_Slow(value.UtcDateTime, buffer, out bytesWritten, encoder);
                case 'O':
                    return TryFormatDateTimeFormatO_Slow(value.UtcDateTime, false, buffer, out bytesWritten, encoder);
                case 'G':
                    return TryFormatDateTimeFormatG_Slow(value.DateTime, buffer, out bytesWritten, encoder);
                default:
                    throw new NotImplementedException();
            }
        }

        public static bool TryFormatSlow(this DateTime value, Span<byte> buffer, out int bytesWritten, TextFormat format = default(TextFormat), TextEncoder encoder = null)
        {
            if (format.IsDefault)
            {
                format.Symbol = 'G';
            ...
            switch (format.Symbol)
            {
                case 'R':
                    ...
                    return TryFormatDateTimeRfc1123_Slow(utc, buffer, out bytesWritten, encoder);
                case 'O':
                    return TryFormatDateTimeFormatO_Slow(value, true, buffer, out bytesWritten, encoder);
                case 'G':
                    return TryFormatDateTimeFormatG_Slow(value, buffer, out bytesWritten, encoder);
                default:
                    throw new NotImplementedException();
            }      
        }

        static bool TryFormatDateTimeFormatG_Slow(DateTime value, Span<byte> buffer, out int bytesWritten, TextEncoder encoder)
        {
        ...
        static bool TryFormatDateTimeFormatO_Slow(DateTimeOffset value, bool isDateTime, Span<byte> buffer, out int bytesWritten, TextEncoder encoder)
        {
            // for now it only works for invariant culture
            if (!encoder.IsInvariantUtf16 && !encoder.IsInvariantUtf8)
            {
            ...
            else
            {
                if (!TryWriteChar('+', buffer, ref bytesWritten, encoder)) { return false; }
                int bytes;
                if (!value.Offset.TryFormatSlow(buffer.Slice(bytesWritten), out bytes, t, encoder)) { return false; }
                bytesWritten += bytes;
            }
        ...
        static bool TryFormatDateTimeRfc1123_Slow(DateTime value, Span<byte> buffer, out int bytesWritten, TextEncoder encoder)
        {
            // for now it only works for invariant culture
            if (!encoder.IsInvariantUtf16 && !encoder.IsInvariantUtf8)
            {
        ...
        public static bool TryFormatSlow(this TimeSpan value, Span<byte> buffer, out int bytesWritten, TextFormat format = default(TextFormat), TextEncoder encoder = null)
        {
            if (format.IsDefault)
            {
                format.Symbol = 'c';
            ...
            if (format.Symbol != 't')
            {
                return TryFormatTimeSpanG_Slow(value, buffer, out bytesWritten, format, encoder);
            }

            // else it's format 't' (short time used to print time offsets)
            return TryFormatTimeSpanT_Slow(value, buffer, out bytesWritten, encoder);
        }

        private static bool TryFormatTimeSpanG_Slow(TimeSpan value, Span<byte> buffer, out int bytesWritten, TextFormat format = default(TextFormat), TextEncoder encoder = null)
        {
            bytesWritten = 0;
        ...
        private static bool TryFormatTimeSpanT_Slow(TimeSpan value, Span<byte> buffer, out int bytesWritten, TextEncoder encoder)
        {
            bytesWritten = 0;
