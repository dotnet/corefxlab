...
namespace System.Buffers
{
    ...
    {
        public static bool TryParseGuid(ReadOnlySpan<byte> text, out Guid value, out int bytesConsumed, ParsedFormat format = default(ParsedFormat))
        {
            if (format.IsDefault) format = 'D';
            switch (format.Symbol)
            {
                case 'D': return TryParseGuidCore(text, false, ' ',  ' ', out value, out bytesConsumed);
                ...
                case 'N': return TryParseGuidN(text, out value, out bytesConsumed);
        ...
        private static bool TryParseGuidN(ReadOnlySpan<byte> text, out Guid value, out int bytesConsumed)
        {
            ...
            int justConsumed;
            if (!Hex.TryParseUInt32(text.Slice(0, 8), out uint i1, out justConsumed) || justConsumed != 8) return false; // 8 digits
        ...
        private static bool TryParseGuidCore(ReadOnlySpan<byte> text, bool ends, char begin, char end, out Guid value, out int bytesConsumed)
        {
            ...
            int justConsumed;
            if (!Hex.TryParseUInt32(text, out uint i1, out justConsumed)) return false;
    ...
    {
        public static bool TryParseGuid(ReadOnlySpan<char> text, out Guid value, out int charactersConsumed, ParsedFormat format = default(ParsedFormat))
        {
            if (format.IsDefault) format = 'D';
            switch (format.Symbol)
            {
