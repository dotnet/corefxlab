...
namespace System.Text
{
    public static partial class PrimitiveParser
    {
        ...
        public static bool TryParseInt32(ReadOnlySpan<byte> text, out int value, out int bytesConsumed, TextFormat format = default(TextFormat), TextEncoder encoder = null)
        {
            ...
            if (!encoder.TryParseSymbol(ref textByte, out uint symbol, out int consumed))
            {
                goto FalseExit;
            }
            ...
            {
                if (!encoder.TryParseSymbol(ref Unsafe.Add(ref textByte, index), out symbol, out consumed) || IsInvalid(symbol))
                {
                    goto Done;
                }
            ...
            if (encoder.TryParseSymbol(ref Unsafe.Add(ref textByte, textLength - 1), out symbol, out consumed) && IsInvalid(symbol))
            {
