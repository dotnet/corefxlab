    ...
    internal sealed partial class DefaultBinder : Binder
    {
        ...
        public sealed override MethodBase SelectMethod(BindingFlags bindingAttr, MethodBase[] match, Type[] types, ParameterModifier[] modifiers)
        {
            ...
            for (i = 0; i < types.Length; i++)
            {
                ...
                if (!(IsImplementedByTypeLoader(realTypes[i]) || realTypes[i].IsSignatureType()))
            ...
            for (i = 0; i < candidates.Length; i++)
            {
                ...
                for (j = 0; j < types.Length; j++)
                {
                    ...
                    if (pCls.IsPrimitive)
                    {
                        if (!(IsImplementedByTypeLoader(type.UnderlyingSystemType)) ||
            ...
            for (i = 0; i < candidates.Length; i++)
            {
                if (indexes != null)
                {
                    ...
                    for (j = 0; j < indexesLength; j++)
                    {
                        ...
                        if (pCls.IsPrimitive)
                        {
                            if (!(IsImplementedByTypeLoader(indexes[j].UnderlyingSystemType)) ||
                ...
                if (j == indexesLength)
                {
                    if (returnType != null)
                    {
                        ...
                        {
                            if (!(IsImplementedByTypeLoader(returnType.UnderlyingSystemType)) ||
