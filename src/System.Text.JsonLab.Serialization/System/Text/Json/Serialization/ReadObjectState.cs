...
{
    internal struct ReadObjectState
    {
        ...
        public static IEnumerable GetEnumerableValue(in ReadObjectState current)
        {
        ...
        public void SetReturnValue(object value, JsonSerializerOptions options)
        {
            Debug.Assert(ReturnValue == null);
            ReturnValue = value;
        }

        public static void SetReturnValue(object value, JsonSerializerOptions options, ref ReadObjectState current, bool setPropertyDirectly = false)
        {
            if (current.IsEnumerable())
            {
            ...
            else if (!setPropertyDirectly && current.IsPropertyEnumerable())
            {
                ...
                {
                    ((IList)current.PropertyInfo.GetValueAsObject(current.ReturnValue, options)).Add(value);
                }
            }
            else
            {
